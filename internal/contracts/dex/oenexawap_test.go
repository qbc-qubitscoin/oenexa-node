...
func TestOenSwap_AliasAndSwapBforA(t *testing.T) {
	os := NewOenSwap()
	if os.ReserveA != 0 || os.ReserveB != 0 {
		t.Fatal("expected empty reserves in new OenSwap")
	}

	// SwapBforA on empty pool
	if _, ok := os.SwapBforA(100); ok {
		t.Fatal("expected false for SwapBforA on empty pool")
	}

	os.AddLiquidity(10_000, 10_000)

	// Zero input
	if _, ok := os.SwapBforA(0); ok {
		t.Fatal("expected false for SwapBforA with 0 input")
	}

	// SwapBforA empty ReserveA
	emptyA := NewOenSwap()
	emptyA.ReserveB = 1000
	if _, ok := emptyA.SwapBforA(100); ok {
		t.Fatal("expected false when ReserveA is 0")
	}

	// SwapBforA empty ReserveB
	emptyB := NewOenSwap()
	emptyB.ReserveA = 1000
	if _, ok := emptyB.SwapBforA(100); ok {
		t.Fatal("expected false when ReserveB is 0")
	}

	// Valid SwapBforA
	outA, ok := os.SwapBforA(1000)
	if !ok || outA == 0 {
		t.Fatalf("expected successful SwapBforA, got %d, %v", outA, ok)
	}
	if os.ReserveB != 11_000 {
		t.Errorf("ReserveB: want 11000, got %d", os.ReserveB)
	}
	if os.ReserveA != 10_000-outA {
		t.Errorf("ReserveA: want %d, got %d", 10_000-outA, os.ReserveA)
	}
}
