...
func TestNewShield_Fields(t *testing.T) {
	cm := [crypto.HashSize]byte{1, 2, 3}
	enc := []byte("encrypted-note")
	tx := NewShield(testWallet.Address, testWallet.PublicKey, 2, 500, MinGasPrice, cm, enc)
	if tx.Type != TxShield {
		t.Errorf("Type: want TxShield, got %d", tx.Type)
	}
	if tx.Amount != 500 {
		t.Errorf("Amount: want 500, got %d", tx.Amount)
	}
	if tx.GasLimit != GasShield {
		t.Errorf("GasLimit: want %d, got %d", GasShield, tx.GasLimit)
	}
}

func TestNewUnshield_Fields(t *testing.T) {
	nf := [crypto.HashSize]byte{4, 5, 6}
	proof := []byte("zk-proof")
	tx := NewUnshield(testWallet2.Address, testWallet.PublicKey, 3, 300, MinGasPrice, nf, proof)
	if tx.Type != TxUnshield {
		t.Errorf("Type: want TxUnshield, got %d", tx.Type)
	}
	if tx.Amount != 300 {
		t.Errorf("Amount: want 300, got %d", tx.Amount)
	}
	if tx.GasLimit != GasUnshield {
