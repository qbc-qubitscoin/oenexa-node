# OENEXA Wallet Architecture Design

**Version**: v1.0
**Phase**: 8

## 1. Overview
The OENEXA Wallet ecosystem encompasses Mobile (iOS/Android), Web, and Desktop applications. Given OENEXA's quantum-safe mandate, the primary challenge is securely storing and managing large keys (ML-DSA-65 private keys are 4,032 bytes, signatures are 3,309 bytes).
...
- **Communication**: JSON-RPC over HTTPS/WSS to OENEXA nodes.
...
- **Node Integration**: Desktop wallets will have the option to run a light client or a full bundled `oenexa-node` in the background for enhanced privacy.
...
OENEXA integrates Threshold Signature Schemes (TSS) adapted for lattice-based cryptography. 
...
On-chain multi-sig is implemented via OenexaVM (WASM).
