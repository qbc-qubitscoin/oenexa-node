# OENEXA DeFi Ecosystem Architecture

**Version**: v1.0
**Phase**: 11 (DEX Development)

## 1. Overview
The OENEXA DeFi ecosystem centers around two core protocols deployed on OenexaVM: **OenexaSwap** (AMM DEX) and **OenexaLend** (Over-collateralized lending market).

## 2. OenexaSwap (AMM)
OenexaSwap utilizes a constant-product formula (`x * y = k`). 
...
## 3. OenexaLend (Money Market)
OenexaLend allows users to lock up volatile assets (e.g., OEN) as collateral to borrow stable assets.
- **Oracle Integration**: Collateral values are strictly priced via the Phase 10 **OenexaOracle**, relying on the median-aggregator to prevent flash-crash liquidations.
...
**Strategic Decision**: OENEXA will **NOT** implement an algorithmic stablecoin mechanism (e.g., seigniorage shares or endogenous collateral like Terra/UST).

**Regulatory Rationale**: Algorithmic stablecoins carry immense regulatory risk and have been heavily targeted by financial regulators (e.g., the US SEC, EU MiCA). To ensure institutional adoption and protect user funds from death spirals, the official OENEXA stablecoin ($QUSD) will operate strictly as a **1:1 Fiat-Reserve-Backed** token.
