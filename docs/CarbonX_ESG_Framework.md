...
OENEXA CarbonX employs a strict **Two-Way Bridge Protocol** to ensure 1:1 parity and absolute prevention of double-counting:

1. **Tokenization (Bridging On-Chain)**: 
   When a carbon credit is bridged to OENEXA, the underlying asset on the traditional registry (Verra/Gold Standard) must be immediately transferred to a dedicated, audited "Custody Wallet/Account" and mathematically locked (or explicitly marked as "Bridged to OENEXA"). It cannot be traded off-chain while the token exists.
   
2. **On-Chain Retirement (Burning)**: 
   When a corporate entity buys the tokenized carbon credit on the OENEXA DEX and wishes to claim the offset, they execute the `RetireCredit` smart contract function.
