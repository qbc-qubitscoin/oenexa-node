...
Ethereum Virtual Machine (EVM) suffers from 256-bit word overhead, expensive gas modeling, and non-standard bytecode. OENEXA uses **WebAssembly (WASM)** powered by **Tetratelabs Wazero**:
...
When a WASM module runs on OEN, the node exposes 6 host functions into the module under the `oen` namespace:

| Host Function | Signature | Purpose | Gas Cost |
|---|---|---|---|
| `oen_get(slot uint32)` | `uint64` | Reads a 64-bit value from contract storage slot | 200 gas |
| `oen_set(slot uint32, val uint64)` | `void` | Writes a 64-bit value into contract storage slot (fails in ReadOnly mode) | 5,000 gas |
| `oen_caller()` | `uint64` (ptr) | Writes the 32-byte address of the transaction sender into WASM linear memory | 50 gas |
| `oen_value()` | `uint64` | Returns the amount of oenexa transferred with the call | 20 gas |
| `oen_block_height()` | `uint64` | Returns the current chain height | 20 gas |
| `oen_log(ptr uint32, len uint32)` | `void` | Emits a debug/event log string from contract linear memory | 500 gas |
...
### 1. OenexaSwap AMM (`internal/contracts/dex/oenexawap.go`)
...
- `SubmitProposal(title, description, requestedOEN, target)`
...
### 4. OenexaID Sovereign Identity (`internal/contracts/oenexaid/oenexaid.go`)
